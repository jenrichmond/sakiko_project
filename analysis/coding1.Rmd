...
```{r summarystats}
summarise(renamedata, meanfamiliar = mean(familiar), sdfamiliar = sd(familiar), meanunfamiliar = mean(unfamiliar), sdunfamiliar = sd(unfamiliar))
...
longdata <- renamedata %>%
  ...
  pivot_longer(names_to = "stimulus", values_to = "looking_time", "familiar":"unfamiliar")
...
plottest <- read_csv(here("data", "1_Mehr2016_plot_test.csv"))
